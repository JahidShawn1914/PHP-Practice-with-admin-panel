...
// todays news
$tnews = "SELECT news.ID, news.Title, news.Icon, news.Description
FROM `news` WHERE C_ID = 23 ORDER BY ID DESC LIMIT 3";

$t_sql = $conn->query($tnews);
  ...
  <body class="bg-light">
    ...
    <section class="tech">
      <div class="container">
        ...
        <div class="row">
          <?php if($t_sql->num_rows > 0){ ?>
          <?php while($t_news = $t_sql->fetch_assoc()){ ?>
          <div class="col-lg-4">
            <div class="c_news text-center">
              <i class="<?php echo $t_news['Icon'];?>" aria-hidden="true"></i>
              <h2><?php echo $t_news['Title'];?></h2>
              <p><?php 

                $data = $t_news['Description'];

                if(strlen($data) > 100){

                  echo $data = substr($data, 0, 100)."..." ."<br>";
                  echo "<a href='details.php?ID=".$t_news['ID']."' class='text-dark mt-3 d-block'>read more</a>";
                }
